...
const updateIcon = updateInfo.querySelector('#icon');
...
ipcRenderer.on('update-status', (event, data) => {
    ...
    if (data.icon) {
        fs.readFile(path.join(__dirname, `../assets/images/${data.icon}.svg`), (err, fileData) => {
            if (err) {
                log.error(err);
            }
            else {
...
ipcRenderer.on('presence-data', (event, data) => {
...
ipcRenderer.on('toggle-discord-reconnect', (event, toggle) => {
